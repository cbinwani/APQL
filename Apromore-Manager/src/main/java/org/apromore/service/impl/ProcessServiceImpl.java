...
public class ProcessServiceImpl extends AbstractObservable implements ProcessService {
    ...
    public ProcessModelVersion importProcess(final String username, final Integer folderId, final String processName,
        ...
        try {
            ...
            LOGGER.error(">>>>>>>>>>>>>>>>>>>>>>IMPORT: "+ processName+" "+process.getId());//call when net is change and then save
    ...
    public ProcessModelVersion updateProcess(final Integer processId, final String processName, final String originalBranchName,
        ...
        try {
            if (!StringUtils.equals(originalBranchName, newBranchName)) {
                ...
                notifyObserver(user,nativeType,pmv,false);
                LOGGER.error(">>>>>>>>>>>>>>>>>>>>>>UPDATE: ", processName);//call when net is change and then save
                ...
                if (user == null) {
                    ...
                    notifyObserver(user,nativeType,pmv,false);
                    LOGGER.error(">>>>>>>>>>>>>>>>>>>>>>UPDATEEXISTINGPROCESS: ", processName);//call when a net is created, change version
                } else {
...
//        for( RequestParameterType rpt : canoniserProperties){
//            LOGGER.error("canoniserProperties: "+rpt.getId()+" "+rpt.getName()+" "+rpt.getClass()+" "+rpt.getDescription()+" "+rpt.isMandatory()+" "+rpt.getValueType()+" "+rpt.getValue());
//        }
//        LOGGER.error("PROCESSSERVIUCE: Dati proc"+name+" "+processId+" "+branch+" "+version+" "+format+" "+annName+" "+withAnn+" "+canoniserProperties);
//        LOGGER.error("EXPORT PROCESS: "+format);
        try {
            ...
            try {
                ...
                if (pvid != null) {
                    ...
                    if (branches.size() > 1 || (branches.size() == 1 && pvid.getProcessBranch().getProcessModelVersions().size() > 1)) {
                        ...
                        Version version=new Version(pvid.getVersionNumber());
                        LOGGER.debug("DELETE METHOD");
//                        notifyObserver(null, null, pvid, true);
                    }
                }
            } catch (Exception e) {
    ...
    private void updateBranch(List<ProcessModelVersion> pmvs, ProcessModelVersion pvid, ProcessBranch branch) throws ExceptionDao {
        ...
        deleteProcessModelVersion(pvid);
        LOGGER.debug("UPDATEBRANCH METHOD");
//        notifyObserver(null, null, pvid, true);
    }
    ...
    private void deleteProcessModelVersion(final ProcessModelVersion pmv) throws ExceptionDao {
        try {
            ...
            notifyObserver(null, null, pmv, true);
            LOGGER.error("DELETEPMV: "+pmv.getProcessBranch().getId());
        } catch (Exception e) {
